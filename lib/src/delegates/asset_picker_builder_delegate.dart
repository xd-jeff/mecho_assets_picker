...
import 'package:mecho_assets_picker/src/assets/images.dart';
...
abstract class AssetPickerBuilderDelegate<Asset, Path> {
  ...
  final String? permissionLimitTip;
...
class DefaultAssetPickerBuilderDelegate
    ...
    if (isWeChatMoment) {
      ...
      if (index == null) {
        if (revert) {
          ...
          list = List.of(p.selectedAssets);
        }
        effectiveIndex = selected.indexOf(currentAsset);
        current = list;
      } else {
        current = List.of(p.currentAssets);
  ...
  Widget androidLayout(BuildContext context) {
    // 实现1.0.7选择路径UI样式
    final appbarLayout = appBar(context);
    final bodyLayout = Consumer<DefaultAssetPickerProvider>(
    ...
    final pathLayout = pathEntityListWidget(context);

    return Material(
      type: MaterialType.transparency,
      child: Stack(
        children: <Widget>[
          Positioned.fill(bottom: null, child: appbarLayout),
          Positioned.fill(
            top: MediaQuery.paddingOf(context).top +
                appbarLayout.preferredSize.height,
            ...
            decoration: BoxDecoration(
              border: Border.all(
                  color: Color(0xff687571)
                      .withValues(alpha: isSelectedNotEmpty ? 1 : 0.5)),
              color: Color(0xffE5F7F1)
                  .withValues(alpha: isSelectedNotEmpty ? 1 : 0.5),
              // gradient: isSelectedNotEmpty
  ...
  Widget pathEntityListWidget(BuildContext context) {
    ...
    return Positioned.fill(
      top: isAppleOS(context)
          ? context.topPadding + appBarPreferredSize!.height - 10
          : 0,
      bottom: null,
                        ...
                        child: BackdropFilter(
                          ...
                          child: ListView.separated(
                            ...
                            separatorBuilder: (_, __) => Container(
                              height: 0,
                              color: Color(0xff6E7082).withValues(alpha: 0.8),
                              child: Container(
              ...
              child: ScaleText(
                ...
                style: TextStyle(
                    color: p.isSelectedNotEmpty
                        ? Color(0xff08C284)
                        : Color(0xff08C284).withValues(alpha: 0.5),
                    fontSize: 14,
                    fontWeight: FontWeight.bold),
                maxScaleFactor: 1.2,
  ...
  Widget selectIndicator(BuildContext context, int index, AssetEntity asset) {
    ...
    return Selector<DefaultAssetPickerProvider, DefaultAssetPickerProvider>(
      selector: (_, DefaultAssetPickerProvider p) => p,
      builder: (BuildContext context, DefaultAssetPickerProvider p, __) {
        final int selectedIndex = p.selectedAssets.indexOf(asset);
        ...
        final Widget innerSelector = AnimatedContainer(
          ...
          child: FittedBox(
            child: AnimatedSwitcher(
              ...
              child: selectedIndex >= 0 ? Text('${selectedIndex + 1}') : null,
            ),
          ),
        );
